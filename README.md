...
| File | Purpose |
|------|---------|
| `agent_tools.py` | Main entry point. Parses Zeek `conn.log` and Suricata `eve.json`, detects scan patterns, and orchestrates the four-pillar triage pipeline. |
| `embed.py` | Embedding pipeline and RAG layer. Loads the CVE database, generates and persists vector embeddings via Ollama, and exposes `analyze_by_port()` - the core LLM verdict function. |
| `iprep.py` | IP reputation tool. Checks a source IP against AbuseIPDB and Shodan InternetDB, with a local SQLite cache to avoid redundant API calls. |
| `database.py` | SQLAlchemy model and session setup for `cves.db`. |
| `api_keys.py` | API key configuration.(make your own) **Do not commit real keys — add to `.gitignore`.** |
| `requirements.txt` | Python dependencies. Install with `pip install -r requirements.txt`. |
| `cves.db` | SQLite database of port-confirmed CVEs with CVSS scores. |
| `embeddings.db` | SQLite database of vector embeddings for each CVE. Generated by `embed.py` on first run. |
| `ip_reputation.db` | Local cache of IP reputation lookups. Auto-generated by `iprep.py`. |
| `README.md` | This file. |
...
- [ ] Move to ChromaDB instead of repopulating the sqlite3 db all the time. 
## License 
MIT :D
